...
class GCN(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        # x6 = self.conv4(x5, edge_index) + x5
        return x4
...
class LatencyModel(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        # Filter nodes with index >= 32
        mask = torch.arange(x.size(0), device=x.device) >= 32
        gcn_output_filtered = gcn_output[mask]
        batch_filtered = batch[mask]
        
        # Apply global pooling
        pooled_output = global_add_pool(gcn_output_filtered, batch_filtered)
        
        x = F.relu(self.lin1(pooled_output))
